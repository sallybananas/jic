...
$.get("/api/session").then(function (session) {
  ...
  $.get(`/api/main/${user.currentUser.id}`).then(function (userData) {
    ...
    for (var i = 0; i < userData.Profile.length; i++) {
      ...
      var cardContentdelete = $(`<i class="material-icons small right tooltipped delete" data-position="top" data-delay="50" data-tooltip="Delete my Account" data-id="${userData.Profile[i]._id}" >remove_circle</i>`)
